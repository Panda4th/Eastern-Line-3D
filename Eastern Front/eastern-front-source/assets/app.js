(()=>{
'use strict';
const D=HISTORY, T=THREE;
const stage=document.getElementById('stage'), labels=document.getElementById('labels'), canvas=document.getElementById('scene'), dateEl=document.getElementById('date'), chapterEl=document.getElementById('chapter'), eventEl=document.getElementById('event'), noteEl=document.getElementById('note'), seek=document.getElementById('seek'), timeEl=document.getElementById('time'), endEl=document.getElementById('end');
const black=0x111318,red=0xc92535;
let renderer;try{renderer=new T.WebGLRenderer({canvas,antialias:true,alpha:true,preserveDrawingBuffer:true})}catch(e){document.getElementById('nowebgl').style.display='block';canvas.style.display='none';['seek','play','restart','speed','view','chapters'].forEach(id=>document.getElementById(id).disabled=true);return}
...
const gBody=new T.BoxGeometry(.63,.22,.39),gTrack=new T.BoxGeometry(.7,.16,.13),gTurret=new T.BoxGeometry(.30,.16,.26),gGun=new T.BoxGeometry(.44,.055,.055);function tank(mat,track){const g=new T.Group();const box=(x,y,z,geo,m)=>{const o=new T.Mesh(geo,m);o.position.set(x,y,z);g.add(o)};box(0,.13,0,gBody,mat);box(0,.075,-.25,gTrack,track);box(0,.075,.25,gTrack,track);box(-.04,.31,0,gTurret,mat);box(.29,.34,0,gGun,mat);return g}
...
let currentSec=0,playing=true,speed=1,azimuth=-.1,elevation=.91,zoom=1,autoCamera=true,lastIndex=-1,lastEnd=false,lastDateStr='',lastEvent=null,stageW=stage.clientWidth,stageH=stage.clientHeight;if(typeof matchMedia==='function'&&matchMedia('(prefers-reduced-motion: reduce)').matches)playing=false;
...
function drawRibbon(front){dc.clearRect(0,0,CW,CH);dc.save();dc.clip(landPath);
...
function placeLabel(el,p,dy=0){const q=p.clone();q.y=.35;world.localToWorld(q);q.project(camera);const w=stageW,h=stageH;el.style.left=((q.x*.5+.5)*w)+'px';el.style.top=((-q.y*.5+.5)*h+dy)+'px';el.style.visibility=q.z>1||q.x<-1||q.x>1||q.y<-1||q.y>1?'hidden':'visible'}
...
for(const p of pockets){const on=currentSec>=p.startT&&currentSec<p.endT;p.ring.visible=p.unit.visible=on;p.el.style.display=on?'block':'none';if(on)placeLabel(p.el,p.pos,24)}
...
const large=stageW>800;cityLabels.forEach(c=>{c.el.style.display=!large&&![0,4,5,6,7,9].includes(c.i)?'none':'block';c.el.classList.toggle('highlight',Math.hypot(c.p.x-focusRing.position.x,c.p.z-focusRing.position.z)<2.5);placeLabel(c.el,c.p,-18)});seaLabels.forEach(c=>placeLabel(c.el,c.p));
compass.style.transform='rotate('+(-azimuth*180/Math.PI)+'deg)';
const date=new Date(s.ms);const text=date.toISOString().slice(0,10).replaceAll('-',' / ');dateEl.textContent=text;
if(lastIndex!==s.i){chapterEl.textContent=String(s.i+1).padStart(2,'0')+' / '+D.keyframes.length;document.getElementById('chapters').value=s.i;lastIndex=s.i}
seek.value=currentSec;if(text!==lastDateStr){seek.setAttribute('aria-valuetext',text);lastDateStr=text}timeEl.textContent=Math.floor(currentSec/60)+':'+String(Math.floor(currentSec%60)).padStart(2,'0')+' / '+Math.floor(total/60)+':'+String(total%60).padStart(2,'0');
const rawDate=date.toISOString().slice(0,10);const event=D.events.findLast(e=>e.date<=rawDate);if(event!==lastEvent){eventEl.textContent=event.title;noteEl.textContent=event.note;lastEvent=event}
const endOn=currentSec>=endT;endEl.style.opacity=endOn?Math.min(1,(currentSec-endT)/1.5):0;if(endOn!==lastEnd){endEl.setAttribute('aria-hidden',String(!endOn));lastEnd=endOn}
}
function resize(){if(!stage.clientWidth||!stage.clientHeight)return;stageW=stage.clientWidth;stageH=stage.clientHeight;renderer.setSize(stageW,stageH,false);camera.aspect=stageW/stageH;camera.updateProjectionMatrix();frame(currentSec)}
seek.max=total;seek.addEventListener('input',()=>{playing=false;syncPlay();frame(+seek.value)});
...
function syncPlay(){document.getElementById('play').textContent=playing?'一時停止':'再生'}
...
function setAuto(v){autoCamera=v;document.getElementById('view').textContent=autoCamera?'真上から見る':'斜めから見る'}document.getElementById('view').onclick=()=>{setAuto(!autoCamera);azimuth=0;elevation=autoCamera?.97:1.55;zoom=1;frame(currentSec)};
let drag=null;canvas.onpointerdown=e=>{if(drag)return;drag={x:e.clientX,y:e.clientY,az:azimuth,el:elevation};setAuto(false);canvas.setPointerCapture(e.pointerId)};canvas.onpointermove=e=>{if(!drag)return;azimuth=drag.az-(e.clientX-drag.x)*.004;elevation=Math.max(.48,Math.min(1.55,drag.el+(e.clientY-drag.y)*.004));frame(currentSec)};canvas.onpointerup=canvas.onpointercancel=canvas.onlostpointercapture=()=>drag=null;
...
canvas.addEventListener('keydown',e=>{if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();setAuto(false);azimuth+=(e.key==='ArrowLeft'?.08:e.key==='ArrowRight'?-.08:0);elevation=Math.max(.48,Math.min(1.55,elevation+(e.key==='ArrowUp'?.05:e.key==='ArrowDown'?-.05:0)));frame(currentSec)}});
D.sources.forEach(s=>{const a=document.createElement('a');a.href=s.url;a.textContent=s.title;a.target='_blank';a.rel='noopener';a.setAttribute('aria-label',s.title+'（新しいタブで開く）');const li=document.createElement('li');li.append(a);document.getElementById('sources').append(li)});
window.animation={seek:sec=>{playing=false;syncPlay();frame(sec)},duration:total,state:()=>({time:currentSec,date:dateEl.textContent,chapter:eventEl.textContent,tanks:troops.filter(t=>t.obj.visible).length}),ready:true};
